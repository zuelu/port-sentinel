...
1. 从 [v1.4.1 Release](https://github.com/zuelu/port-sentinel/releases/tag/v1.4.1) 下载 **PortSentinel-v1.4.1-windows-x64.zip**，解压到可写的固定目录。
   Download **PortSentinel-v1.4.1-windows-x64.zip** from the [v1.4.1 release](https://github.com/zuelu/port-sentinel/releases/tag/v1.4.1) and extract it to a stable, writable folder.
...
以下展示 v1.4.1 的 9 个功能页面，均使用虚构演示数据。截图中的地址、连接、规则和主机信息仅用于说明界面，不代表真实流量或相关服务的行为。点击图片可查看大图。

These screenshots show all nine feature pages in v1.4.1 using fictional sample data. Addresses, connections, rules, and host details illustrate the interface only; they do not represent actual traffic or the behavior of the named services. Click an image to view it at full size.
